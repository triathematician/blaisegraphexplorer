...
import org.bm.graphexplorer.controllers.AbstractGraphController;
...
public class GraphListModel extends AbstractListModel
    ...
    public Object getNodeAt(int index) {
        return primaryNodes == null ? null : primaryNodes.get(index);
    }
    ...
    public Object getElementAt(int index) {
        ...
        if (active instanceof ValuedGraph && active.contains(node))
            return node.toString() + ": " + ((ValuedGraph)active).getValue(node);
        return node;
    }

    public void propertyChange(PropertyChangeEvent evt) {
        if (evt.getSource() == gc) {
            if (evt.getPropertyName().equals("baseGraph"))
                fireContentsChanged(this, 0, getSize());
        }
    }
