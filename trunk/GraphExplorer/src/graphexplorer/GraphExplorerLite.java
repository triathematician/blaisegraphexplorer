...
public class GraphExplorerLite extends javax.swing.JFrame
    ...
    private void updatePropertyPanel() {
        ...
        if (manager != null)
            propertyRP.add(new PropertySheet(manager), "Graph");
//        PlaneGraphBean pgb = new PlaneGraphBean(manager);
...
//        propertyRP.add(new PropertySheet(pgb.labelBean()), "Label Settings");
    ...
    private void initComponents() {
        ...
        mainPlot = new org.bm.blaise.specto.plane.graph.GraphComponent();
    ...
    private void mainTMTableChanged(javax.swing.event.TableModelEvent evt) {//GEN-FIRST:event_mainTMTableChanged
        ...
        // TODO - restore support for labels
...
//            manager.setLabel(evt.getFirstRow(), (String) mainTM.getValueAt(evt.getFirstRow(), evt.getColumn()));
    ...
    */
    ...
    private org.bm.blaise.specto.plane.graph.GraphComponent mainPlot;
    ...
    public GraphComponent graphPlot() { return mainPlot; }
    public Graph activeGraph() {
        return manager == null ? null : manager.getGraph();
    }

    public Map<Object, Point2D.Double> getActivePoints() {
        return manager == null ? null : manager.getNodePositions();
    }

    public void setActivePoints(Map<Object, Point2D.Double> pos) {
        if (manager != null)
            manager.requestPositionMap(pos);
    }
    ...
    public void loadGraph(Graph sg, String name) {
        closeActiveGraph();
        mainPlot.setGraphManager(manager = new GraphManager(sg));
        newPositions = null;
        updatePropertyPanel();
...
//            mainTM.setGraph(sg);
    ...
    public void loadLongitudinalGraph(TimeGraph lg, String name) {
    ...
    public void closeActiveGraph() {
        stopLayout();
        if (manager != null)
            mainPlot.setGraphManager(manager = null);
...
//        mainTM.setGraph(null);
    ...
    public <V> void initLayout(StaticGraphLayout layout, double... parameters) {
        stopLayout();
        activeLayout = null;
        newPositions = layout.layout(manager.getGraph(), parameters);
        manager.requestPositionMap(newPositions);
    ...
    public void initLayout(IterativeGraphLayout layout) {
        ...
        if (activeGraph() != null) {
            if (activeLayout == null || activeLayout != layout)
                activeLayout = layout;
            layout.reset(manager.getNodePositions());
            newPositions = null;
        }
    ...
    public void adjustLayout(StaticGraphLayout layout, double... parameters) {
        if (manager != null && activeGraph() != null)
    ...
    public void iterateLayout() {
        ...
        activeLayout.iterate(manager.getGraph());
        manager.requestPositionMap(activeLayout.getPositions());
    }
    ...
    public void stateChanged(ChangeEvent e) {
        if (manager != null && e.getSource() == manager) {
            newPositions = manager.getNodePositions();
        }
    }
