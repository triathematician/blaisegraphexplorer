...
//                }
    ...
    public GraphType saveGraph(Object go, Map<Object,Point2D.Double> positions, File file) {
        try {
            if (go instanceof TimeGraph) {
                saveLongitudinal((TimeGraph<Integer>) go, positions, file);
    ...
    private void saveLongitudinal(TimeGraph<Integer> lg, Map<Object,Point2D.Double> positions, File file) {
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(file));
            try {
                // write the vertices
